...
import com.enniu.qa.apm.service.AgentPackageService;
import com.enniu.qa.apm.service.LocalAgentService;
 ...
 * Agent manager.
 * <p/>
 * This class has {@link AgentControllerServerDaemon} internally and host to the agent connection.
 *
 * @author JunHo Yoon
 * @since 3.0
 */
...
public class AgentManager implements ControllerConstants, AgentDownloadRequestListener {
	...
	public AgentIdentity getAgentByName(String name) {
		for (AgentIdentity agentIdentity : getAllAttachedAgents()) {
			if (StringUtils.equals(convert(agentIdentity).getName(), name)) {
	...
	public Set<AgentIdentity> filterApprovedAgents(Set<AgentIdentity> agents) {
		...
		for (GrinderAgentInfo each : cachedLocalAgentService.getLocalAgents()) {
			if (each.isApproved()) {
	...
	/*public synchronized void runAgent(User user, final SingleConsole singleConsole,
		...
		running(grinderProperties,singleConsole,necessaryAgents);
	}*/

	/*
	*  compatible with multi-api case running at the same time ,e.g. whole service scalability test
	* */
	public synchronized void runAgent(final User user, final SingleConsole singleConsole, final GrinderProperties grinderProperties, final Set<AgentIdentity> useAgents) {
		final Set<AgentIdentity> allFreeAgents = getAllFreeApprovedAgentsForUser(user);

		Set<AgentIdentity> availables = checkAgentAvailable(user, allFreeAgents, useAgents);
		if (availables.size() == 0) {
			LOGGER.error("No agent available, stop running.");
			return;
		} else if (availables.size() < useAgents.size()) {
			LOGGER.warn("Not enough agents,but still running.");
		}

		running(grinderProperties,singleConsole,availables);

	}


	public void running(final GrinderProperties grinderProperties,SingleConsole singleConsole,Set<AgentIdentity> availables){
	...
	private Set<AgentIdentity> checkAgentAvailable(User user, Set<AgentIdentity> allFreeAgents, Set<AgentIdentity> useAgents) {
		Set<AgentIdentity> agentIdentities = new HashSet<AgentIdentity>();

		for (AgentIdentity each : allFreeAgents) {
			String region = ((AgentControllerIdentityImplementation) each).getRegion();
			if (StringUtils.endsWith(region, "owned_" + user.getUserId()) || !StringUtils.contains(region, "owned_")) {
