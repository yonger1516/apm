...
@Component
public class PerfTestRunnable2 implements ControllerConstants {

	private static final Logger LOG = LoggerFactory.getLogger(PerfTestRunnable2.class);
	...
	void doStart() {
		...
		if (!canExecuteMore()) {
			// LOG MORE
			List<ApiTestRun> currentlyRunningTests = perfTestService2.getCurrentlyRunningTest();
			LOG.debug("Currently running test is {}. No more tests can not run.", currentlyRunningTests.size());
			return;
		}
		// Find out next ready perftest
		ApiTestRun runCandidate = getRunnablePerfTest();
	...
	private ApiTestRun getRunnablePerfTest() {
		return perfTestService2.getNextRunnablePerfTestPerfTestCandidate();
	}

	private boolean canExecuteMore() {
		return consoleManager.getConsoleInUse().size() < perfTestService2.getMaximumConcurrentTestCount();
	}

	private boolean isScheduledNow(ApiTestRun test) {
		ApiRunTestConfig config=test.getRunConfig();

		Date current = new Date();
		Date scheduledDate = DateUtils
				.truncate((Date) defaultIfNull(config.getScheduledTime(), current), Calendar.MINUTE);
	 ...
	 */
	protected boolean hasEnoughFreeAgents(ApiTestRun test) {
		int size = agentManager.getAllFreeApprovedAgentsForUser(test.getCreatedUser()).size();
		ApiRunTestConfig config=test.getRunConfig();
		if (config.getAgentCount() != null && config.getAgentCount() > size) {
			perfTestService2.markProgress(test, "The test is tried to execute but there is not enough free agents."
					+ "\n- Current free agent count : " + size + "  / Requested : " + config.getAgentCount() + "\n");
			return false;
		}
		return true;
	}
	 ...
	 */
	public void doTest(final ApiTestRun perfTest) {
		SingleConsole singleConsole = null;
		try {
			singleConsole = startConsole(perfTest);
			ScriptHandler prepareDistribution = perfTestService2.prepareDistribution(perfTest);
			GrinderProperties grinderProperties = perfTestService2.getGrinderProperties(perfTest, prepareDistribution);
			...
			singleConsole.setReportPath(perfTestService2.getReportFileDirectory(perfTest));
	 ...
	 */
	SingleConsole startConsole(ApiTestRun perfTest) {
		perfTestService2.markStatusAndProgress(perfTest, START_CONSOLE, "Console is being prepared.");
		// get available consoles.
		ConsoleProperties consoleProperty = perfTestService2.createConsoleProperties(perfTest);
		SingleConsole singleConsole = consoleManager.getAvailableConsole(consoleProperty);
		singleConsole.start();
		perfTestService2.markPerfTestConsoleStart(perfTest, singleConsole.getConsolePort());
		return singleConsole;
	}
	 ...
	 */
	void distributeFileOn(final ApiTestRun perfTest, SingleConsole singleConsole) {
		// Distribute files
		perfTestService2.markStatusAndProgress(perfTest, DISTRIBUTE_FILES, "All necessary files are being distributed.");
		...
		listener.add(new SingleConsole.FileDistributionListener() {
			...
			public void distributed(String fileName) {
				perfTestService2.markProgress(perfTest, " - " + fileName);
			}

			@Override
			public boolean start(File dir, boolean safe) {
				if (safe) {
					perfTestService2.markProgress(perfTest, "Safe file distribution mode is enabled.");
					return safe;
				}
				long sizeOfDirectory = FileUtils.sizeOfDirectory(dir);
				if (sizeOfDirectory > safeThreadHold) {
					perfTestService2.markProgress(perfTest, "The total size of distributed files is over "
		...
		singleConsole.distributeFiles(perfTestService2.getDistributionPath(perfTest), listener,
				isSafeDistPerfTest(perfTest));
		perfTestService2.markStatusAndProgress(perfTest, DISTRIBUTE_FILES_FINISHED,
	...
	private boolean isSafeDistPerfTest(final ApiTestRun perfTest) {
		boolean safeDist = getSafe(perfTest.getRunConfig().getSafeDistribution());
	 ...
	 * @param singleConsole     console to be used.
	 */
	void startAgentsOn(ApiTestRun perfTest, GrinderProperties grinderProperties, SingleConsole singleConsole) {
		ApiRunTestConfig config=perfTest.getRunConfig();

		perfTestService2.markStatusAndProgress(perfTest, START_AGENTS,
				"agents are starting.");

		//to support multi-api test
		Set<AgentIdentity> agentIdentities=new HashSet<AgentIdentity>();
		if (!perfTest.getRunConfig().getAgentId().isEmpty()){
			agentIdentities.add(agentManager.getAgentByName(perfTest.getRunConfig().getAgentId()));//specify agent to run
		}else{
			agentIdentities=agentManager.selectAgent(perfTest.getCreatedUser(),agentManager.getAllFreeApprovedAgentsForUser(perfTest.getCreatedUser()),perfTest.getRunConfig().getAgentCount());//random to select numbers fo agent to run
		}
		agentManager.runAgent(perfTest.getCreatedUser(), singleConsole, grinderProperties,
				agentIdentities);
		singleConsole.waitUntilAgentConnected(config.getAgentCount());
		perfTestService2.markStatusAndProgress(perfTest, START_AGENTS_FINISHED, getSafe(config.getAgentCount())
				+ " agents are ready.");
	}
	 ...
	 * @param singleConsole     console to be used.
	 */
	void runTestOn(final ApiTestRun perfTest, GrinderProperties grinderProperties, final SingleConsole singleConsole) {

		// Run test
		perfTestService2.markStatusAndProgress(perfTest, START_TESTING, "The test is ready to start.");
		...
		singleConsole.addListener(new ConsoleShutdownListener() {
			...
			public void readyToStop(StopReason stopReason) {
				perfTestService2.markAbnormalTermination(perfTest, stopReason);
				LOG.error("Abnormal test {} due to {}", perfTest.getId(), stopReason.name());
			}
		});
		long startTime = singleConsole.startTest(grinderProperties);
		perfTest.setCreatedDate(new Date(startTime));
		addSamplingListeners(perfTest, singleConsole);
		perfTestService2.markStatusAndProgress(perfTest, TESTING, "The test is started.");
		singleConsole.startSampling();

	}

	protected void addSamplingListeners(final ApiTestRun perfTest, final SingleConsole singleConsole) {
		...
		singleConsole.addSamplingLifeCyleListener(new AgentDieHardListener(singleConsole, perfTest, perfTestService2,
	 ...
	 */
	public void notifyFinish(ApiTestRun perfTest, StopReason reason) {
	...
	void doFinish() {
		for (ApiTestRun each : perfTestService2.getAllAbnormalTesting()) {
		...
		for (ApiTestRun each : perfTestService2.getAllStopRequested()) {
		...
		for (ApiTestRun each : perfTestService2.getAllTesting()) {
			SingleConsole consoleUsingPort = consoleManager.getConsoleUsingPort(each.getPort());
	 ...
	 * @param perfTest perfTest
	 */
	private void cleanUp(ApiTestRun perfTest) {
		perfTestService2.cleanUpDistFolder(perfTest);
		perfTestService2.cleanUpRuntimeOnlyData(perfTest);
	}
	 ...
	 */
	private boolean isTestFinishCandidate(ApiTestRun perfTest, SingleConsole singleConsoleInUse) {
		ApiRunTestConfig config=perfTest.getRunConfig();
		// Give 5 seconds to be finished
		if (config.isThresholdDuration()
				&& singleConsoleInUse.isCurrentRunningTimeOverDuration(config.getDuration())) {
			LOG.debug(
					"Test {} is ready to finish. Current : {}, Planned : {}",
					new Object[]{perfTest.getTestIdentifier(), singleConsoleInUse.getCurrentRunningTime(),
							config.getDuration()});
			return true;
		} else if (config.isThresholdRunCount()
				&& singleConsoleInUse.getCurrentExecutionCount() >= config.getTotalRunCount()) {
	 ...
	 *                           {@link PerfTest}
	 */
	public void doCancel(ApiTestRun perfTest, SingleConsole singleConsoleInUse) {
		LOG.info("Cancel test {} by user request.", perfTest.getId());
		singleConsoleInUse.unregisterSampling();
		try {
			perfTestService2.markProgressAndStatusAndFinishTimeAndStatistics(perfTest, CANCELED,
	 ...
	 *                           {@link PerfTest}
	 */
	public void doTerminate(ApiTestRun perfTest, SingleConsole singleConsoleInUse) {
		singleConsoleInUse.unregisterSampling();
		try {
			perfTestService2.markProgressAndStatusAndFinishTimeAndStatistics(perfTest, Status.STOP_BY_ERROR,
					"Stopped by error");
		} catch (Exception e) {
	 ...
	 *                           {@link PerfTest}
	 */
	public void doNormalFinish(ApiTestRun perfTest, SingleConsole singleConsoleInUse) {
		...
		try {
			...
			if (perfTestService2.hasTooManyError(perfTest)) {
				perfTestService2.markProgressAndStatusAndFinishTimeAndStatistics(perfTest, Status.STOP_BY_ERROR,
				...
				perfTestService2.markProgressAndStatusAndFinishTimeAndStatistics(perfTest, Status.FINISHED,
						"The test is successfully finished.");
			}
		} catch (Exception e) {
			perfTestService2.markStatusAndProgress(perfTest, Status.STOP_BY_ERROR, e.getMessage());
	...
	public PerfTestService2 getPerfTestService2() {
		return perfTestService2;
	}
