...
import com.iyonger.apm.web.handler.NullScriptHandler;
import com.iyonger.apm.web.handler.ProcessingResultPrintStream;
import com.iyonger.apm.web.handler.ScriptHandler;
...
import com.iyonger.apm.web.model.PerfTestStatistics;
import com.iyonger.apm.web.model.ProcessAndThread;
import com.google.common.collect.Lists;
import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.iyonger.apm.web.dao.PerfTestSpecification;
...
public class PerfTestService extends AbstractPerfTestService implements ControllerConstants, GrinderConstants {
	...
	public Page<PerfTest> getPagedAll(User user, String query, String tag, String queryFilter, Pageable pageable) {
		Specifications<PerfTest> spec = Specifications.where(PerfTestSpecification.idEmptyPredicate());
		// User can see only his own test
		if (user.getRole().equals(Role.USER)) {
			spec = spec.and(PerfTestSpecification.createdBy(user));
		}

		if (StringUtils.isNotBlank(tag)) {
			spec = spec.and(PerfTestSpecification.hasTag(tag));
		}
		if ("F".equals(queryFilter)) {
			spec = spec.and(PerfTestSpecification.statusSetEqual(Status.FINISHED));
		} else if ("R".equals(queryFilter)) {
			spec = spec.and(PerfTestSpecification.statusSetEqual(Status.TESTING));
		} else if ("S".equals(queryFilter)) {
			spec = spec.and(PerfTestSpecification.statusSetEqual(Status.READY));
			spec = spec.and(PerfTestSpecification.scheduledTimeNotEmptyPredicate());
		}
		if (StringUtils.isNotBlank(query)) {
			spec = spec.and(PerfTestSpecification.likeTestNameOrDescription(query));
		}
	...
	List<PerfTest> getAll(User user) {
		Specifications<PerfTest> spec = Specifications.where(PerfTestSpecification.createdBy(user));
	...
	public PerfTest getOne(User user, Long id) {
		...
		spec = spec.and(PerfTestSpecification.idEqual(id));
	...
	public List<PerfTest> getAll(User user, Long[] ids) {
		...
		spec = spec.and(PerfTestSpecification.idSetEqual(ids));
		return perfTestRepository.findAll(spec);
	}
	...
	public long count(User user, Status[] statuses) {
		...
		if (statuses.length == 0) {
			...
			return perfTestRepository.count(spec.and(PerfTestSpecification.statusSetEqual(statuses)));
		}
	...
	public List<PerfTest> getAll(User user, Status[] statuses) {
		...
		if (statuses.length != 0) {
			spec = spec.and(PerfTestSpecification.statusSetEqual(statuses));
		}
	...
	private List<PerfTest> getAll(User user, String region, Status[] statuses) {
		...
		if (config.isClustered()) {
			spec = spec.and(PerfTestSpecification.idRegionEqual(region));
		}
		if (statuses.length != 0) {
			spec = spec.and(PerfTestSpecification.statusSetEqual(statuses));
		}
